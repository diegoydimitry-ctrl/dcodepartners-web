...
// ── GUARDA GOOGLE ADS (Fase 0, 18/09/2026) ─────────────────────────────────
// Regla de Dirección: un lead con Fuente = google_ads NO genera propuesta
// automática. Queda como BORRADOR INTERNO hasta que haya diagnóstico humano
// (Etapa embudo = Reunión celebrada o posterior). Falla cerrado: si no se
// puede leer la fuente, se trata como google_ads.
const fuente = String(campo(lead, 'Fuente') && (campo(lead, 'Fuente').name || campo(lead, 'Fuente')) || '');
const etapaEmbudo = String(campo(lead, 'Etapa embudo') && (campo(lead, 'Etapa embudo').name || campo(lead, 'Etapa embudo')) || '');
const esGoogleAds = fuente === 'google_ads' || (!lead.id);
const diagnosticoHecho = ['Reunión celebrada', 'Propuesta', 'Prueba', 'Cliente'].includes(etapaEmbudo);
const googleAdsOk = !esGoogleAds || diagnosticoHecho;
...
const motivos = [];
if (!googleAdsOk) motivos.push('Lead de Google Ads sin diagnóstico humano: la propuesta queda como BORRADOR INTERNO (regla 18/09/2026).');
...
return [{
  json: {
    ...
    empresa,
    pasaValidaciones: !!lead.id && prioridadOk && contactoOk && sinPropuestaPrevia && noSuprimido && googleAdsOk,
    estadoPropuestaSiGoogleAds: googleAdsOk ? null : 'Borrador interno (pendiente de diagnóstico)',
    motivos
  }
}];