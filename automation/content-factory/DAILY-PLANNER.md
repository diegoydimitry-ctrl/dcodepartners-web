...
| 08:00 | `CF/Concept → Script → Storyboard → Render` | Solo produce las ideas con `formatoDecidido = VIDEO` (o sin decidir aún, por compatibilidad). Gemini escribe guion + decide diseño de audio de esa pieza en concreto (familia musical o silencio intencionado, efectos por escena). Sin voz/TTS — retirada del pipeline automático (DCP-CONTENT-FACTORY-003). Guarda en `CF_Scripts`/`CF_Videos` y dispara el render en GitHub Actions. |
