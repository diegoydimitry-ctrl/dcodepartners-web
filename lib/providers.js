 ...
 * transitorio (503/429/529 del proveedor, timeout, error de red) — ver
 * withRetries() — porque ese es el fallo real más frecuente visto en
 * producción ("Gemini API respondió con estado 503... high demand...
 * usually temporary"). Solo el PRIMER proveedor de la cadena reintenta
 * varias veces: el resto (el/los proveedor(es) de respaldo) se prueba una
 * única vez — ver el parámetro maxAttempts de generate() y su uso en
 * api/chat.js. Esto no es un recorte de fiabilidad, es matemática de
 * presupuesto de tiempo: api/chat.js fija maxDuration=60s; con timeout de
 * 12s por intento, reintentar también el proveedor de respaldo podría
 * superar ese límite y la función moriría a mitad sin poder devolver el
 ...
 * objeto { name, generate(systemPrompt, messages, options) } que
 * devuelva una Promise<string> con la respuesta en texto/Markdown,
 * registrarlo en PROVIDERS/HAS_KEY, y añadirlo al orden por defecto en
 * getProviderChain().
 */
...
// respondiendo. 12s (no 15s) para que el peor caso realista quepa con
// margen dentro de maxDuration=60s de api/chat.js: proveedor principal
// (hasta 3 intentos) + proveedor de respaldo (1 intento) ≈ 3×12 + 1×12 +
// backoff ≈ 50s, dejando ~10s de margen para arranque en frío y red.
...
// Solo se reintenta lo que tiene sentido reintentar: cualquier 5xx
// (sobrecarga/fallo transitorio del lado del proveedor — 503 "high
// demand" de Gemini y 529 "Overloaded" de Anthropic son los casos ya
// vistos en producción, pero un 500/502/504 tiene la misma naturaleza
// transitoria y no hay motivo para tratarlo distinto), 429 (límite de
// tasa, no de cuota total) y fallos de red/timeout. Un 400 (petición mal
// formada) o 404 (modelo inexistente) fallarán exactamente igual en el
// siguiente intento — reintentarlos solo alarga la espera del usuario sin
...
async function withRetries(attemptFn, { maxAttempts = DEFAULT_MAX_ATTEMPTS } = {}) {
  let lastError;
  ...
  // sirve de red de seguridad real si maxAttempts fuera 0 o negativo.
  throw lastError;
}
...
const geminiProvider = {
  ...
  generate(systemPrompt, messages, options) {
    return withRetries(() => callGeminiOnce(systemPrompt, messages), options);
  },
};
...
async function callAnthropicOnce(systemPrompt, messages) {
  const response = await fetchWithTimeout('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      // Sin esto, Claude Sonnet 5 razona internamente por defecto (a
      // diferencia de modelos anteriores, donde omitir 'thinking'
      // significaba "no pensar") y ese razonamiento consume del mismo
      // presupuesto que max_tokens — el mismo riesgo de "respuesta vacía
      // por presupuesto agotado" que ya se vio con Gemini (ver
      // MAX_OUTPUT_TOKENS más arriba), pero sin cubrir aquí. Este endpoint
      // no usa tools, así que no aplica el efecto secundario documentado
      // de desactivar thinking en modelos con tool use.
      thinking: { type: 'disabled' },
      // El system prompt es idéntico entre peticiones consecutivas (hasta
      // el próximo deploy o hasta que expire el TTL de 5 min de la caché)
      // — cache_control permite a Anthropic no volver a facturar por
      // completo ese contexto en cada intento/reintento. Relevante en
      // especial porque Anthropic es el proveedor de RESPALDO: entra en
      // juego justo cuando Gemini ya está fallando, el escenario de mayor
      // volumen hacia este proveedor.
  ...
  if (!text) {
    // stop_reason (end_turn, max_tokens...) es la pista real de por qué no
    // hay texto — igual que finishReason en Gemini, se incluye para poder
    // diagnosticar sin acceso a los logs de Vercel.
...
const anthropicProvider = {
  name: 'anthropic',
  generate(systemPrompt, messages, options) {
    return withRetries(() => callAnthropicOnce(systemPrompt, messages), options);
  },
};
