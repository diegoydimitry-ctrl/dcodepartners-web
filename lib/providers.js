...
// ---------------------------------------------------------------------------
// Reintentos y tiempo máximo por intento. AUD-20260830.
//
// El 24/08 hubo una ráfaga de fallos de /api/chat que llegaron a visitantes
// reales: 49 grupos de error distintos en Vercel, casi todos "Gemini API
// respondió con estado 429" (cuota). La llamada era un único fetch sin
// reintento: un 429 puntual —que en el plan gratuito suele ser un límite por
// minuto y se pasa solo en un par de segundos— se convertía directamente en
// un error visible en el asistente de la web.
//
// Además no había AbortController, así que una llamada colgada corría hasta
// el límite de la plataforma. En los registros hay peticiones de 54 s y 84 s:
// el visitante se queda mirando el chat todo ese tiempo.
//
// Se corrigen las dos cosas sin cambiar de proveedor ni gastar de más: se
// reintenta SOLO lo que tiene sentido reintentar, con un presupuesto total
// acotado. Un 400/401/403 es un fallo de configuración y reintentarlo solo
// añade espera.
const MAX_INTENTOS = 3;
const TIMEOUT_POR_INTENTO_MS = 15000;
const PRESUPUESTO_TOTAL_MS = 40000;
...
/**
 * fetch con tiempo máximo. Si se agota, aborta y lanza un error marcado como
 * reintentable, en vez de dejar la petición colgada.
 */
  ...
  try {
    ...
    const e = new Error(
      err && err.name === 'AbortError'
        ? `La llamada a Gemini superó ${timeoutMs} ms y se abortó`
        : `Fallo de red llamando a Gemini: ${err && err.message ? err.message : err}`
    );
    e.reintentable = true;
    throw e;
  } finally {
...
const geminiProvider = {
  ...
  async generate(systemPrompt, messages) {
    ...
    // Bucle de reintento. Solo se repite lo reintentable (429, 5xx, timeout,
    // fallo de red) y solo mientras quede presupuesto de tiempo: un visitante
    // esperando en el chat no puede quedarse ahí indefinidamente.
    ...
    for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
      try {
        response = await fetchConTimeout(url, opciones, TIMEOUT_POR_INTENTO_MS);
        ...
        const err = new Error(
          `Gemini API respondió con estado ${response.status}: ${errorBody.slice(0, 500)}`
        );
        err.reintentable = ESTADOS_REINTENTABLES.has(response.status);
        response = null;
        throw err;
      } catch (err) {
        ultimoError = err;
        ...
        // Se descuenta también lo que puede tardar el PROXIMO intento: si no,
        // el presupuesto se comprueba antes de gastar 15 s mas y el peor caso
        // se dispara muy por encima del limite que se pretendia fijar.
        const hayPresupuesto =
          Date.now() - inicio + espera + TIMEOUT_POR_INTENTO_MS <= PRESUPUESTO_TOTAL_MS;
        if (!err.reintentable || !quedanIntentos || !hayPresupuesto) throw err;
        console.warn(
          `[chat] Gemini falló en el intento ${intento}/${MAX_INTENTOS}, reintentando en ${espera} ms: ` +
    ...
    if (!response) throw ultimoError || new Error('Gemini no devolvió ninguna respuesta');
