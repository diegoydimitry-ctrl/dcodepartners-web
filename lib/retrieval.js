...
const STOPWORDS = new Set([
  ...
  // Determinantes interrogativos: marcan el TIPO de pregunta, no el tema.
  // "cuánto" no debería, por sí solo, hacer coincidir "¿cuántos años
  // tienes?" con un fragmento sobre "veinte años de trayectoria".
...
// Subconjunto más estricto: el usuario quiere específicamente hablar con
// alguien o dejar sus datos, no solo mostrar interés comercial genérico
// (que puede tener una respuesta propia mejor, p. ej. la del precio en la
// FAQ). Solo este subconjunto debe forzar que aparezca el contacto directo
// en el cuerpo de la respuesta — el resto se queda en el CTA suave del final.
const CONTACT_KEYWORDS = [
  'hablar con', 'contactar', 'contacto', 'agendar', 'cita', 'reunion', 'llamada',
];
...
function detectContactIntent(query) {
  const norm = normalize(query);
  return CONTACT_KEYWORDS.some((kw) => norm.includes(kw));
}
...
function loadIndex() {
  ...
  // Se indexa como una página más, sin URL real (pageUrl: null — no hay
  // nada que enlazar) para que el asesor pueda responder con conocimiento
  // técnico general, no solo con contenido del propio sitio. Los alias se
  // añaden al heading (nunca se muestra al usuario, ver buildExtractiveAnswer)
  // solo para reforzar la búsqueda: "qué es make" también debe encontrarlo.
  const generalPage = {
    url: null,
    title: 'Conocimiento técnico general',
    chunks: generalKnowledge.map((entry, i) => ({
      id: 'general#c' + i,
      heading: entry.aliases && entry.aliases.length ? `${entry.heading} ${entry.aliases.join(' ')}` : entry.heading,
      text: entry.text,
    })),
  };
  cachedIndex = buildIndex({ pages: [...kb.pages, generalPage] });
  return cachedIndex;
}

module.exports = { tokenize, buildIndex, search, detectBuyingIntent, detectContactIntent, loadIndex };
